    ...
    for (WVJBMessage* message in messages) {
        ...
        if (responseId) {
            ...
            if (callbackId) {
                ...
                // 没有callbackId的情况是，js调用方法但没有传入回调函数
            ...
            // 原代码中如果H5调用了OC没有注册的方法s时，并没有其他任何处理，针对此问题，此处手动维护优化逻辑：如果H5调用了OC中没注册的方法，则统一回调报错方法
            if (!handler) {
                NSLog(@"WVJBNoHandlerException, No handler for message from JS: %@", message);
                responseCallback(@{ @"code":@(-9999)});
                //1. js中必须要传入回调函数，这样才能保证js能接收非法方法名调用的回调
                //2. 统一OC回传jJSON格式，包括一个code 参数
                //3. js的回调中优先判断code字段，如果为-9999再继续操作，因为旧版本中有返回空内容情况
            } else {
